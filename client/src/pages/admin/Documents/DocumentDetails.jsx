...
export default function DocumentDetails() {
    ...
    const handleReupload = () => {
        ...
        if (doc?.purpose === 'activities') {
            reuploadRejectedActivityDocumentMutate({ formData, documentId: doc._id },
                {
                    onSuccess: () => {
                        setLoading(false);
        ...
        if (doc?.purpose === 'recognition') {
            reuploadRejectedAccreditationDocumentMutate({ formData, documentId: doc._id },
                {
                    onSuccess: () => {
                        setLoading(false);
                        setReuploadModalOpen(false);
        ...
        console.log("uploading ", file, "and document id ", doc._id);
            ...
            <div className="flex flex-col items-center justify-start w-full  px-3 sm:px-4 lg:px-6 pb-16">
                <div className="flex flex-col w-full max-w-5xl justify-center mb-6">
                    ...
                    <div className='w-full flex flex-col sm:flex-row justify-between gap-4 items-end'>
                        ...
                        {/* upload doc action */}
                        {statusDisplay.label === 'Rejected' && isUserRSORepresentative && (
                            <Button onClick={() => setReuploadModalOpen(true)}>
                                <div
                                    data-tooltip-id="global-tooltip"
                                    data-tooltip-content='Upload new document'
                                    className='flex gap-2 items-center'>
                                    <svg xmlns="http://www.w3.org/2000/svg" className='size-4 fill-white' viewBox="0 0 640 640"><path d="M342.6 73.4C330.1 60.9 309.8 60.9 297.3 73.4L169.3 201.4C156.8 213.9 156.8 234.2 169.3 246.7C181.8 259.2 202.1 259.2 214.6 246.7L288 173.3L288 384C288 401.7 302.3 416 320 416C337.7 416 352 401.7 352 384L352 173.3L425.4 246.7C437.9 259.2 458.2 259.2 470.7 246.7C483.2 234.2 483.2 213.9 470.7 201.4L342.7 73.4zM160 416C160 398.3 145.7 384 128 384C110.3 384 96 398.3 96 416L96 480C96 533 139 576 192 576L448 576C501 576 544 533 544 480L544 416C544 398.3 529.7 384 512 384C494.3 384 480 398.3 480 416L480 480C480 497.7 465.7 512 448 512L192 512C174.3 512 160 497.7 160 480L160 416z" /></svg>
                                    Replace File
                                </div>
                            </Button>
                        )}
                            ...
                            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
                                <div className="bg-white rounded-lg shadow-2xl w-full max-w-md mx-4 overflow-hidden">
                                    <div className="px-4 py-3 border-b border-gray-200 flex items-center justify-between">
                                        <h3 className="text-base font-semibold text-gray-900">Replace File</h3>
                                        <CloseButton onClick={() => { setReuploadModalOpen(false); setFile(null); }} />
                                    </div>
                                            ...
                                            (<div {...getRootProps()} className='w-full py-4 bg-gray-100 border border-mid-gray rounded flex justify-center cursor-pointer hover:bg-gray-200'>
                                                ...
                                                <div className='w-full flex flex-col gap-4'>
                                                    <div
                                                        ...
                                                        className='flex items-center justify-between border border-mid-gray rounded px-4 py-2 gap-4'>
                                                        <h1 className='font-semibold truncate max-w-[60%]'>{file.name}</h1>
                                                        <div className='flex items-center gap-2'>
                                                            <p className='text-sm text-gray-600'>Size: {(file.size / (1024 * 1024)).toFixed(2)} MB</p>
                                                            <svg onClick={() => setFile(null)} xmlns="http://www.w3.org/2000/svg" className='fill-gray-600 size-6 cursor-pointer hover:fill-gray-400' viewBox="0 0 640 640"><path d="M320 576C461.4 576 576 461.4 576 320C576 178.6 461.4 64 320 64C178.6 64 64 178.6 64 320C64 461.4 178.6 576 320 576zM231 231C240.4 221.6 255.6 221.6 264.9 231L319.9 286L374.9 231C384.3 221.6 399.5 221.6 408.8 231C418.1 240.4 418.2 255.6 408.8 264.9L353.8 319.9L408.8 374.9C418.2 384.3 418.2 399.5 408.8 408.8C399.4 418.1 384.2 418.2 374.9 408.8L319.9 353.8L264.9 408.8C255.5 418.2 240.3 418.2 231 408.8C221.7 399.4 221.6 384.2 231 374.9L286 319.9L231 264.9C221.6 255.5 221.6 240.3 231 231z" /></svg>
                                                        </div>
                                                    </div>
                                                </div>
                                            )}
                                    ...
                                    <div className="px-4 py-3 border-t border-gray-200 flex items-center justify-end gap-2 bg-white">
                                        <Button
                                            disabled={loading}
                                            onClick={() => { handleReupload(); setLoading(true); }}>{loading ? <LoadingSpinner /> : `Upload and Replace`}</Button>
                                    </div>
                                </div>
                            </div>
                        )}
