...
import { ActivityDeadlineBanner, Backdrop, Button, CloseButton, FormReviewModal, LoadingSpinner, ReusableTable, TabSelector, TextInput, UploadBatchModal } from '../../../components';
...
export default function Activities() {
  ...
  console.log("activity for responses: ", activityFormsResponses);
  ...
  useEffect(() => {
    if (!(activityDocumentsLoading || viewAdminActivityLoading)) {
      const timer = setTimeout(() => setShowContent(true), 1000);
      return () => clearTimeout(timer);
    } else {
      setShowContent(false);
    }
  }, [activityDocumentsLoading, viewAdminActivityLoading]);
  ...
  console.log("Activity forms responses on role: ", activityFormsResponses);
  ...
  const [selectedForms, setSelectedForms] = useState("pre");
  ...
  return (
    <>
      {(isUserRSORepresentative ? activityDocumentsLoading : viewAdminActivityLoading) ? (
        <div className='w-full px-4 sm:px-8 lg:px-24 mt-6'>
          {/* Skeleton Loader for Activity Header */}
          <div className='flex flex-col justify-start gap-4 items-start w-full md:justify-between md:items-center md:flex-row animate-pulse'>
            <div className='flex flex-col md:flex-row items-center gap-4 w-full'>
              <div className='flex items-center gap-2'>
                <div className='aspect-square h-12 w-12 bg-gray-200 rounded-full'></div>
          ...
          <div className='bg-dark-gray rounded-md flex justify-center items-center w-full'>
            <div className='aspect-square w-full sm:w-[60%] md:w-[50%] lg:w-[40%] bg-gray-200 overflow-hidden'></div>
          </div>
          {/* Skeleton Loader for Info Badges */}
        ...
        <div className="flex flex-col items-start min-h-[150vh]">
          ...
          <div className='w-full px-4 sm:px-8 lg:px-24 mt-6'>
            ...
            <div className='mt-8 w-full'>
              ...
              {activeTab === 2 && (
                <div className="w-full mt-4">
                  {(!isUserRSORepresentative ? isLoadingSpecificActivityFormsResponse : isLoadingSpecificActivityFormsResponse) ? (
                    <div className="flex justify-center items-center h-32">
                      <LoadingSpinner></LoadingSpinner>
                    </div>
                  ) : (activityFormsResponses?.preRegData || activityFormsResponses?.feedbackData)?.length > 0 ? (
                    <div className="bg-white border border-gray-300 rounded-lg p-6">
                      <div className='flex justify-start mb-6 gap-4'>
                        <button
                          onClick={() => setSelectedForms('pre')}
                          className={` rounded hover:underline text-sm ${selectedForms === 'pre' ? 'font-bold underline text-gray-700' : 'text-gray-600'}`}
                        >
                          {`Pre-Registration Responses (${activityFormsResponses?.metadata?.preRegistrationResponses || 0})`}
                        </button>
                        <button
                          onClick={() => setSelectedForms('feedback')}
                          className={` rounded hover:underline text-sm ${selectedForms === 'feedback' ? 'font-bold underline text-gray-700' : 'text-gray-600'}`}
                        >
                          {`Feedback Responses (${activityFormsResponses?.metadata?.feedbackResponses || 0})`}
                        </button>
                      </div>
                      <ul className="space-y-2">
                        {selectedForms === 'pre' ? (
                          activityFormsResponses?.preRegData?.length > 0 ? (
                            activityFormsResponses?.preRegData || []).map((p, index) => (
                              <li
                                data-tooltip-id="global-tooltip"
                                data-tooltip-content={`${p.studentId.firstName} ${p.studentId.lastName}`}
                                onClick={() => openProfileModal({ ...p, index })}
                                key={p._id} className="cursor-pointer text-sm text-gray-800 border border-gray-200 rounded-md px-4 py-2 bg-gray-50 hover:bg-gray-100">
                                {p.studentId.firstName} {p.studentId.lastName}
                              </li>
                            )) : (
                            <span className="text-sm text-gray-600">No pre-registration responses available.</span>
                          )) : (selectedForms === 'feedback') ? (
                            activityFormsResponses?.feedbackData?.length > 0 ? (
                              activityFormsResponses?.feedbackData || []).map((p, index) => (
                                <li
                                  data-tooltip-id="global-tooltip"
                                  data-tooltip-content={`${p.studentId.firstName} ${p.studentId.lastName}`}
                                  onClick={() => openProfileModal({ ...p, index })}
                              ...
                              <span className="text-sm text-gray-600">No feedback responses available.</span>
                            )) : (
                          <span className="text-sm text-gray-600">No responses available.</span>
                        )}
                      </ul>
                    </div>
                  ) : (
